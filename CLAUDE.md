...
- **`skills/dev/`** — a **Layer 3 executor leaf** binding the `run` primitive + `exit-zero`/`empty-output`/`judged` predicates to real process/filesystem execution. `conductor.go` is a goal-oriented orchestrator that drives agent CLIs (via `bashy weave …`); `goverify.go`, `gitskills.go`, `judge.go` are domain leaves.
